...
function _DBPermission {
    ...
    $query = " `
      IF SUSER_ID('$($script:remote_nodes)') IS NULL CREATE LOGIN [$($script:remote_nodes)] FROM WINDOWS `
      GO
      GRANT CONNECT ON ENDPOINT::[Hadr_endpoint] TO [$($script:remote_nodes)] `
      GO `
    ...
    try {
      Write-Logger "$Global:hostname - Granting permission to endpoint"
      Write-Logger $query
...
function JoinDomain {
    ...
    $SA_PASSWORD = (ConvertTo-SecureString (_FetchFromMetaData `
     -property 'attributes/c2d-property-sa-password') -AsPlainText -Force)
...
function SetScriptVar {
    ...
    $script:sa_password = _FetchFromMetaData -property 'attributes/c2d-property-sa-password'

    # Get all nodes
    $Script:all_nodes = ((_FetchFromMetaData -property 'attributes/sql-nodes').split("|")).Where({ $_ -ne "" })

    # Add FQDN and get static ip address
    $ip_count = 1
    ForEach ($host_node in $all_nodes) {
     $Script:all_nodes_fqdn += "$host_node.$Script:domain_name"
     $Script:static_ip += "10.$ip_count.1.4"
     $Script:static_listner_ip += "10.$ip_count.1.5/255.255.0.0"
     $ip_count++
...
function UpdateSubWaiter {
   <#
    ...
    CreateRunTimeVariable -config_path (_GetRuntimeConfig) -var_name "$key_path" /failure/failed
    #TODO remove the following 2 lines
    $path=_GetRuntimeConfig
    Write-Logger "RuntimeConfigPath is $path"
}
