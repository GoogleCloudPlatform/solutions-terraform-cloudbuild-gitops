...
def instance_notification(event, context):
    ...
    demo_project        = os.environ.get('DEMO_PROJECT', 'Specified environment variable is not set.')
    ...
    try:
        ...
        for search_asset in search_asset_result:
            ...
            if search_asset['name'] == message_json["asset"]["name"]:
                send_slack_chat_notification(message_json["asset"]["name"])

    except Exception as error:
        print(f"Error in listing non-compliant instances: {error}")

def send_slack_chat_notification(assetName):
    try:
        slack_message = [
                {
                    "type": "header",
